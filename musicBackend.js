...
const client = new Client({
    intents: [
        ...
        // TEMP: GuildMembers removed — pending Discord privileged intent verification.
        // Re-add once approved in the Developer Portal: https://discord.com/developers/applications
    ...
    const updateStatus = () => {
        ...
        client.user.setPresence({
            ...
            activities: [
                {
                    name: 'Custom Status',
                    state: `/help • with ${serverCount} servers `,
...
// Patakbuhin ang Roblox Sniper sa sarili nitong proseso
const path = require('path');
const { spawn } = require('child_process');

const sniperDir = path.join(__dirname, 'roblox-sniper'); // <-- Palitan ang 'roblox-sniper' kung iba ang pangalan ng folder mo

const sniper = spawn('node', ['index.js'], {
  cwd: sniperDir,
  stdio: 'inherit',
  env: process.env
});
...
sniper.on('error', (err) => {
  console.error('Failed to start Roblox Sniper:', err);
});
